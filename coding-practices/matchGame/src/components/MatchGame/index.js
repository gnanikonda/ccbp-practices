...
class MatchGame extends Component {
  ...
  startTiemr = () => {
    this.timeInterval = setInterval(() => {
      this.setState(a => ({timer: a.timer - 1}))
    }, 1000)
  }
  ...
  getfilteredItems = () => {
    ...
    const filteredTabItems = imagesList.filter(
      each => each.category === activeTabId.tabId,
    )
    return filteredTabItems
  }

  onChangeActiveId = id1 => {
    this.setState(a => ({
      isActive: !a.isActive,
      activeTabId: id1,
    }))
  ...
  onAddToClickeItem = x => {
    ...
    if (bigImage.id === x) {
      this.setState(k => ({score: k.score + 1}))
    } else {
      this.setState({isRunning: false})
    }
    const randomNbr = Math.ceil(Math.random() * 30)
    this.setState(prevState => ({
      bigImage: imagesList[randomNbr],
      clickedItems: [...prevState.clickedItems, x],
    }))
  }
  ...
  displayElemnts = () => {
    const filerdItems = this.getfilteredItems()
    const {activeTabId, clickedItems, bigImage, timer, score} = this.state
    ...
    return (
      <div>
        <div className="abc">
          <img src={bigImage.imageUrl} alt="match" className="thumbnailClass" />
        </div>
        ...
        <ul className="ulclass2">
          {filerdItems.map(b => (
            <TabItems
              onAddToClickeItem={this.onAddToClickeItem}
              details1={b}
              key={b.id}
            />
          ))}
      ...
      <div className="scorecard">
        ...
        <div className="resetdiv">
          <img
            ...
            className="reset"
          />
          <button className="playagain" onClick={this.restartGame}>
            Play Again
          </button>
        </div>
      </div>
        ...
        <ul>
          <nav className="navclass">
            ...
            <li className="scorediv">
              <p className="timercolor1">
                Score: <span className="spanclass">{score}</span>
              </p>
              <div className="timerImagediv">
                <img
                ...
                <p className="timercolor">{timer} sec</p>
              </div>
            </li>
          </nav>
        </ul>
        <div className="bgcontainer">
          {isRunning ? this.displayElemnts() : this.stopGame()}
        </div>
      </div>
    )
  }
